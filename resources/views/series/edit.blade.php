    ...
    <form method="POST" action="{{ route('series.update', $series->id) }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('nama_series') is-invalid @enderror" id="nama_series"
                name="nama_series" value="{{ old('nama_series', $series->nama_series) }}">
            @error('nama_series')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('tipe_series') is-invalid @enderror" id="tipe_series"
                name="tipe_series" value="{{ old('tipe_series', $series->tipe_series) }}">
            @error('tipe_series')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('target_pengguna') is-invalid @enderror"
                id="target_pengguna" name="target_pengguna"
                value="{{ old('target_pengguna', $series->target_pengguna) }}">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control @error('tahun_rilis') is-invalid @enderror" id="tahun_rilis"
                name="tahun_rilis" value="{{ old('tahun_rilis', $series->tahun_rilis) }}">
            @error('tahun_rilis')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="number" class="form-control @error('generasi') is-invalid @enderror" id="generasi"
                name="generasi" value="{{ old('generasi', $series->generasi) }}">
            @error('generasi')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
            ...
            <select class="form-select @error('brand_id') is-invalid @enderror" id="brand_id" name="brand_id">
                ...
                @foreach ($brands as $brand)
                    <option value="{{ $brand->id }}"
                        {{ old('brand_id', $series->brand_id) == $brand->id ? 'selected' : '' }}>
                        {{ $brand->nama_brand }}
                    </option>
                @endforeach
            </select>
